...
def _get_dataset_filename(dataset_dir, split_name, shard_id, filename):
  ...
  filename = temp[0]
  output_filename = 'smgo_%s_%05d-of-%05d-name-%s.tfrecord' % (
...
# lenet
# tf.app.flags.DEFINE_string(
#     'checkpoint_path', '/home/sm/PycharmProjects/smproject/smgo/train',
#     'The directory where the model was written to or an absolute path to a '
#     'checkpoint file.')

# alexnet
tf.app.flags.DEFINE_string(
    'checkpoint_path', '/home/sm/PycharmProjects/smproject/smgo/train_alexnet',
    'The directory where the model was written to or an absolute path to a '
    'checkpoint file.')
...
tf.app.flags.DEFINE_string(
    'dataset_name', 'smgo', 'The name of the dataset to load.')

tf.app.flags.DEFINE_string(
    'dataset_split_name', 'validation', 'The name of the train/test split.')

tf.app.flags.DEFINE_string(
    'dataset_dir', '/home/sm/PycharmProjects/smproject/smgo/testimages', 'The directory where the dataset files are stored.')
...
# alexnet
tf.app.flags.DEFINE_string(
    'model_name', 'alexnet_v2', 'The name of the architecture to evaluate.')
...
# alexnet - vgg
tf.app.flags.DEFINE_string(
    'preprocessing_name', 'vgg', 'The name of the preprocessing to use. If left '
    'as `None`, then the model_name flag is used.')
...
def test():
  ...
  with tf.Graph().as_default():
    ...
    dataset = dataset_factory.get_dataset(
      'smgo', 'validation', '/home/sm/PycharmProjects/smproject/smgo/testimages')
    ...
    network_fn = nets_factory.get_network_fn(
      FLAGS.model_name,
      num_classes=(4 - FLAGS.labels_offset),
      is_training=False)
    ...
    names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
        'faces': slim.metrics.streaming_accuracy(predictions, test1),
        'food': slim.metrics.streaming_accuracy(predictions, test2),
        'nature': slim.metrics.streaming_accuracy(predictions, test3),
        'pets': slim.metrics.streaming_accuracy(predictions, test4)
    })
